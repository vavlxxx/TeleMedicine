...
@pytest.mark.asyncio
async def test_admin_dashboard_returns_users_questions_answers_and_pending_items(ac: AsyncClient, db_session: AsyncSession) -> None:
    await _create_admin(db_session)
    ...
    question = await ac.post('/questions/', json={'text': 'Need help with recurring headaches for 2 weeks.'}, headers=patient_headers)
